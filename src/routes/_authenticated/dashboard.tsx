...
import { CalendarClock, LogOut, Sparkles, Trash2, X, Check, Loader2 } from "lucide-react";
import {
  getMeetings, updateMeetingStatus, deleteMeeting,
  parseSchedulingText, createDirectMeeting, isOwner,
...
function DashboardPage() {
  ...
  const [preview, setPreview] = useState<{ title: string; start_time: string; end_time: string } | null>(null);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="glass rounded-2xl p-8 max-w-md text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">This workspace already has an owner. Sign out and use the owner's account.</p>
          <button onClick={signOut} className="mt-4 h-10 px-4 rounded-lg bg-primary text-primary-foreground">Sign out</button>
        </div>
      </div>
    );
  }
      ...
      <header className="border-b border-border/50 backdrop-blur-lg sticky top-0 z-40 bg-background/80">
        <div className="max-w-[1600px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2">
            <a href="/settings" className="text-sm px-3 py-2 rounded-lg hover:bg-accent">Settings</a>
            <a href="/book" target="_blank" className="text-sm px-3 py-2 rounded-lg hover:bg-accent">Booking page</a>
            <button onClick={signOut} className="text-sm px-3 py-2 rounded-lg hover:bg-accent flex items-center gap-2">
        ...
        <DndContext sensors={sensors} onDragStart={(e) => setActiveId(String(e.active.id))} onDragEnd={handleDragEnd}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Column id="pending" title="Pending" count={pending.length} pulse>
              {pending.map(m => <MeetingCard key={m.id} m={m}
            ...
            <Column id="cancelled" title="History" count={history.length}>
              {history.map(m => <MeetingCard key={m.id} m={m} onDelete={() => deleteMut.mutate(m.id)} />)}
              {history.length === 0 && <EmptyState label="No history yet" />}
            </Column>
          </div>
          <DragOverlay>
            {active ? <MeetingCard m={active} dragging /> : null}
          </DragOverlay>
        </DndContext>
      </main>
      ...
      {preview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="glass rounded-2xl p-6 max-w-md w-full">
            ...
            <p className="text-xs text-muted-foreground mb-4">Review parsed details before saving to Google Calendar.</p>
            <div className="space-y-3 mb-4">
              ...
              <Field label="When">
                <div className="text-sm">
                  {formatDate(preview.start_time)} · {formatTime(preview.start_time)} → {formatTime(preview.end_time)}
                </div>
              </Field>
            </div>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setPreview(null)} className="h-10 px-4 rounded-lg border border-border">Cancel</button>
              <button onClick={confirmPreview} className="h-10 px-4 rounded-lg bg-primary text-primary-foreground glow">Create meeting</button>
            </div>
          </div>
        </div>
      )}
...
function Column({ id, title, count, pulse, children }: { id: string; title: string; count: number; pulse?: boolean; children: React.ReactNode }) {
  const { setNodeRef, isOver } = useDroppable({ id });
  return (
    <div ref={setNodeRef} className={`rounded-2xl border border-border/60 p-4 min-h-[500px] bg-card/40 ${isOver ? "ring-2 ring-primary" : ""} ${pulse ? "pulse-glow" : ""}`}>
...
function MeetingCard({ m, onApprove, onReject, onCancel, onDelete, dragging }: {
  m: Meeting;
  onApprove?: () => void;
  onReject?: () => void;
  onCancel?: () => void;
  ...
  const sourceBadge = { dashboard: "Dashboard", telegram: "Telegram", public_booking: "Website" }[m.source];
  return (
    <div
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <span className="text-[10px] px-2 py-0.5 rounded-full bg-secondary/50 border border-primary/30 text-primary-foreground">{sourceBadge}</span>
      </div>
      <div className="text-xs text-muted-foreground space-y-0.5">
        <div>{formatDate(m.start_time)}</div>
        <div>{formatTime(m.start_time)} → {formatTime(m.end_time)}</div>
        {m.attendee_name && <div className="pt-1 text-foreground">{m.attendee_name}{m.attendee_email && ` · ${m.attendee_email}`}</div>}
        {m.attendee_purpose && <div className="italic">{m.attendee_purpose}</div>}
      </div>
      <div className="flex gap-2 mt-3">
        {onApprove && (
          <button onClick={(e) => { e.stopPropagation(); onApprove(); }} className="flex-1 h-8 text-xs rounded-md bg-primary/90 text-primary-foreground flex items-center justify-center gap-1 hover:bg-primary">
            <Check className="w-3 h-3" /> Approve
          </button>
        )}
        {onReject && (
          <button onClick={(e) => { e.stopPropagation(); onReject(); }} className="flex-1 h-8 text-xs rounded-md border border-border hover:bg-destructive/20 flex items-center justify-center gap-1">
            <X className="w-3 h-3" /> Reject
          </button>
        )}
        {onCancel && (
          <button onClick={(e) => { e.stopPropagation(); onCancel(); }} className="flex-1 h-8 text-xs rounded-md border border-border hover:bg-destructive/20">
            Cancel
          </button>
        )}
        {onDelete && (
          <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="h-8 w-8 rounded-md border border-border hover:bg-destructive/20 flex items-center justify-center">
